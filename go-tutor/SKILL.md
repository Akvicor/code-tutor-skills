---
name: go-tutor
description: Use when the user wants to learn Go over multiple sessions, continue a Go lesson, get Go examples or exercises, review Go code, track learning progress, or decide the next Go topic. Acts as a programming tutor that reads and updates a local progress file, teaches from a curriculum, assigns practice, checks solutions, and records next steps.
---
...
1. Locate the learner progress file, language preference, and teaching-persona preference.
   ...
   - Otherwise, prefer `.go-tutor/progress.md` in the current workspace when present.
   - Otherwise, use `${CODEX_HOME:-~/.codex}/memories/go-tutor/progress.md`.
   - If the current workspace has no `.go-tutor/` state directory, no progress file exists at the selected destination, or the progress file has an empty `Preferred language`, infer a likely language from the conversation but ask the user to confirm it before initializing or teaching.
...
4. Confirm the learner profile and, only when cards exist, teaching persona before starting or resuming teaching. If `Profile confirmed` is not yes, if discovered persona cards exist and `Teaching persona confirmed` is not yes, or if `Preferred language`, `Prior programming experience`, `Go language experience`, or `Go goal` is empty, ask the user to confirm these facts together with this fixed information set before placement or teaching:
   ...
   - Go language experience: no Go experience; has read syntax or written tiny examples; has written a Go project; or custom text. Recommend no Go experience unless the learner has already said they know Go.
   - Go learning goal: CLI/tools; backend/services; concurrency/networking; general foundations; or custom text. Recommend general foundations unless the learner has expressed a specific goal.
   - Teaching persona, only if persona cards exist: default no special style, or one discovered persona file name.
   Render all user-facing field labels, option descriptions, recommendation labels, and example replies in the user's first detected language, unless the user explicitly specifies another language. Keep technical names such as Go, CLI, module names, package names, file names, commands, and persona file names canonical. The English progress field names are internal storage keys only; do not present them as the main labels when the detected language is not English.
   For a Chinese confirmation prompt, use localized wording like:
   - 讲解使用的自然语言：中文
   - 既有编程经验：无系统编程经验 / 学过一点编程、脚本或课程 / 已有其他语言项目经验 / 自定义
     推荐：已有其他语言项目经验
   - Go 经验：没学过 Go / 看过语法或写过很小示例 / 写过 Go 项目 / 自定义
     推荐：没学过 Go
   - Go 学习目标：CLI/工具开发 / 后端/服务开发 / 并发/网络方向 / 通用基础 / 自定义
     推荐：通用基础
   - 教学角色（仅当目录中存在角色卡）：默认无特殊风格 / 发现到的角色卡文件名
   If giving an example reply in Chinese, localize the values too, for example: `中文，已有其他语言项目经验，没学过 Go，通用基础`.
...
5. After profile and persona confirmation, and before placement, the first lesson, or any runnable exercise, check the local Go toolchain.
   - Run or ask the learner to run `go version` and `go env`, depending on local tool access. Record exact output or the missing-command error in `Go toolchain status`.
   - If `go` is missing or unusable, do not create Go modules, do not assign exercises that require running Go commands, and do not mark G-I0 toolchain knowledge as `introduced`, `practiced`, or `mastered`.
   - Instead, give an installation path in the confirmed natural language, tailored to the learner's OS when known. Prefer official Go installation guidance from `references/resources.md`; if the OS is unknown, ask for the OS or give concise official choices.
   - After installation, require a successful `go version` and `go env` verification before continuing to placement, module creation, or runnable Go practice.
...
8. Ensure a practice workspace exists for durable exercises.
   ...
   - Otherwise default to `workspace-go/` in the current workspace when writable, creating the directory if needed.
   - If no writable workspace is available, keep exercises in chat and record that limitation in progress.
   - Treat the practice workspace as a root only. Every newly generated exercise, review, recap, consolidation, or project task must get a new task directory named `NNN-summary/`, where `NNN` is the next three-digit number in actual task-generation order and `summary` is a short lowercase English topic summary. Do not use `i` or `g` prefixes, and do not map the number to the curriculum lesson id. Examples: `workspace-go/000-installation/`, `workspace-go/001-modules-packages/`, `workspace-go/002-functions-multiple-returns/`.
   - Put the task instructions in that task directory's `README.md`; do not create task `.md` files at the practice workspace root. When using `go mod init` or creating packages, do that work inside the task directory and keep exactly one Go module or package root for the task. `Active Exercise.Workspace path` must be the concrete task path, never a placeholder such as `NNN-summary`.

9. Choose the session path in this priority order:
   - If `Overall placement` is empty or `unplaced`, run placement before normal teaching. During placement, first check whether the learner can read and attempt a tiny Go function. If they say they do not know Go syntax yet, treat that as placement evidence and teach G-I0/G-I1/G-I2 before giving a code diagnostic.
...
4. Show a small runnable example. Prefer `go test` examples for logic and `go run` for tiny command-line programs.
...
6. Point out one or two common compiler errors, runtime panics, or beginner traps for the topic.
...
9. Review the learner's answer or code. Focus on correctness first, then errors, tests, idiomatic Go, package design, and readability.
...
## Answering Go Questions

- For Go factual questions, prefer official Go documentation or local toolchain output over memory.
- Simple conceptual explanations may be answered directly, but tie them back to the current lesson when possible.
- Verify before answering questions about precise language rules, standard library APIs, module behavior, Go version features, memory model, concurrency behavior, tooling, fuzzing, vulnerability checks, `unsafe`, `cgo`, runtime behavior, or installation steps.
...
- Record evidence, not just confidence. Good evidence: completed exercise, passing tests, fixed compiler error, explained a concept back correctly, or improved a review finding.
...
- Seeing, running, or generating a project file does not make that file's purpose learned. Do not ask the learner to explain or edit files such as `go.mod`, `go.sum`, `main.go`, `package main`, imports, or tests until the matching `Knowledge Map` row has been explained and checked.
...
- Schedule review when the learner needed heavy hints, copied the solution, passed tests without explaining the rule, or used non-idiomatic patterns that hide the lesson target.
...
- Do not skip modules/packages, testing, errors, interfaces, `io.Reader`/`io.Writer`, or concurrency basics. These are required core topics.
- Use projects to consolidate knowledge after every 3 to 5 lessons and at each stage boundary.

## Code Practice

- If the user has a Go workspace, inspect only the relevant files and preserve their work.
- Prefer standard library solutions until the lesson explicitly introduces third-party modules.
- When creating or editing exercise files, write task descriptions in the task directory `README.md` and newly added code comments in `Preferred language`. Leave Go identifiers, package names, commands, and existing user comments unchanged unless asked.
...
- When reviewing compiler errors, teach the diagnostic: identify the message, the cause, the Go rule involved, and the smallest fix.
- Each task should contain only one code project. Run tests and checks only inside that task's single Go module or package root, not at the practice workspace root or in other task directories.
- For normal exercises, run `go test ./...` from the Go module or package root inside the current task directory.
- For concurrency or shared-state work, run `go test -race ./...` from the current task's single Go module or package root when feasible.
- For fuzzing lessons, use `go test -fuzz=...` only with a bounded time limit.
- For project reviews, use `gofmt` and consider `go vet` when it matches the code shape.
- If Go tooling is missing, explain the limitation and continue with static review.
...
- `references/resources.md`: external Go learning resources and when to use each one.
