...
5. If `Profile confirmed` is not yes, if persona card files exist and `Teaching persona confirmed` is not yes, or if `Preferred language`, `Prior programming experience`, `Go language experience`, or `Go goal` is empty, ask the user to confirm the profile facts and, only when cards exist, the persona before placement or teaching. Use this fixed information set:
...
6. Before placement, the first lesson, or any runnable exercise, check the local Go toolchain by running or asking the learner to run `go version` and `go env`. Record exact output or missing-command errors in `Go toolchain status`.
7. If `go` is missing or unusable, stop before module creation and before runnable Go practice. Give an installation path in the confirmed natural language, using official Go installation guidance from `resources.md`; tailor it to the learner's OS when known, or ask for the OS if needed. Record only "toolchain missing / installation pending" evidence, do not mark G-I0 toolchain knowledge as `introduced`, `practiced`, or `mastered`, and continue only after successful `go version` and `go env` verification.
8. If `Practice workspace` is empty, use `workspace-go/` when writable and record it after profile confirmation.
...
1. Use the confirmed learner profile: prior programming experience, Go goal, and Go language experience. If any item is missing or inferred but not confirmed, ask for confirmation before placing.
2. If the learner says they do not know Go syntax yet, do not give a function/slice/table-driven-test diagnostic. Record a conservative placement result with start lesson G-I0, mark syntax readiness as the evidence, and start teaching G-I0/G-I1/G-I2 in order.
3. If the learner knows general programming but is new to Go syntax, give a brief Go function/module/testing orientation before any diagnostic, then use a tiny function exercise only if they are willing to try it.
4. If the learner can attempt Go code, give one small diagnostic exercise using functions, conditionals, slices, and a table-driven test.
5. For experienced developers, add one question about errors, interfaces, or goroutines before deciding the start point.
...
1. Read progress.
2. Confirm or ask for `Preferred language`, prior programming experience, Go language experience, and Go goal before teaching; handle teaching persona according to the directory-scan rules. Apply the confirmed language to the reply, workspace task description, and any newly added code comments.
3. Verify `Go toolchain status`. If `go` has not been successfully checked in this session or recorded as currently available, run or ask for `go version` and `go env` before any runnable lesson or exercise. If missing, switch to installation guidance and stop before runnable practice.
...
11. Create a fresh task directory named `NNN-summary/` under the practice workspace, using the next three-digit number in actual task-generation order. Do not use lesson-id prefixes. Write the task instructions to `README.md` inside it, and keep exactly one Go module or package root inside that directory. Never create task `.md` files at the practice workspace root.
...
3. Review in this order: correctness, errors, tests, idiomatic Go, package/API shape, readability, concurrency safety when relevant.
4. Explain compiler errors, failing tests, panics, or races with the exact rule involved.
...
3. Include a practical "what this prevents" note for errors, interfaces, concurrency, context, and package design.
...
4. Test core logic separately from IO, network, or CLI glue.
...
- Use `workspace-go/NNN-summary/` for each newly generated exercise, review, recap, consolidation, or project task. `NNN` is the next three-digit number by actual task-generation order, and `summary` is a short lowercase English topic summary. Examples: `workspace-go/000-installation/`, `workspace-go/001-modules-packages/`, `workspace-go/002-functions-multiple-returns/`, `workspace-go/004-intro-recap/`.
- Do not use `i` or `g` prefixes and do not require the directory number to match a curriculum lesson id.
- Write the task description to `workspace-go/NNN-summary/README.md`.
...
- Create exactly one Go module or package root inside the task directory, such as `workspace-go/001-modules-packages/hello/`.
- Run validation only inside that Go module or package root, normally `go test ./...`.
- Do not create task `.md` files directly under `workspace-go/`.
...
1. Restate the goal and relevant Go rule or idiom.
2. Point to the line, test case, or function shape, not the full answer.
3. Give a partial signature, test table row, or interface shape.
...
- Explain errors as ordinary values that callers can inspect or wrap.
- Explain interfaces through consumer needs and implicit implementation.
- Explain concurrency through ownership of goroutines, channels, cancellation, and shared state.
- For each compiler error or failed test, connect the symptom to a stable rule or idiom.
...
- `Current State`
- `Learner Profile`, including `Preferred language`, teaching persona file/default fields, profile/persona confirmation, prior programming experience, Go language experience, and Go goal when selected or changed
